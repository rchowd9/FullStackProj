...
const quests = [
  {
    id: 'systems-incident-budget',
    title: 'Incident Budget',
    difficulty: 'Advanced',
    xp: 620,
    category: 'System Design',
    concept: 'Reliability Engineering',
    prompt: 'Your checkout API has a 99.95% monthly availability target. A deploy is ready, but its error budget is already exhausted. What is the most responsible next move?',
    choices: ['Ship immediately because the target is only an average', 'Pause the feature deploy and prioritize reliability work', 'Delete the availability target', 'Add retries to every client without measuring impact'],
    correctAnswer: 'Pause the feature deploy and prioritize reliability work',
    explanation: 'An exhausted error budget signals that reliability is below the agreed service level. Feature work should pause while the team restores the budget and learns from the incidents.',
    choiceExplanations: {
      'Ship immediately because the target is only an average': 'A monthly average still represents a user-facing reliability contract.',
      'Pause the feature deploy and prioritize reliability work': 'Correct. The error budget gives the team a concrete tradeoff between shipping and stability.',
      'Delete the availability target': 'Removing the measurement hides the problem instead of improving the service.',
      'Add retries to every client without measuring impact': 'Unbounded retries can amplify an outage and create a retry storm.',
    },
  },
  {
    id: 'security-session-replay',
    title: 'Replay Window',
    difficulty: 'Advanced',
    xp: 640,
    category: 'Cybersecurity',
    concept: 'Threat Modeling',
    prompt: 'A payment endpoint accepts a signed request containing a timestamp, but it does not remember request IDs. Which threat remains most plausible?',
    choices: ['A replay attack using a previously valid request', 'A SQL query becoming faster', 'A cache miss on a public asset', 'A compiler optimizing the signature'],
    correctAnswer: 'A replay attack using a previously valid request',
    explanation: 'A valid signature proves authenticity, but without freshness or request-id tracking, an attacker may resend a captured request.',
    choiceExplanations: {
      'A replay attack using a previously valid request': 'Correct. Nonces, timestamps with a narrow window, and idempotency records help close this gap.',
      'A SQL query becoming faster': 'Query speed is unrelated to whether a signed request can be reused.',
      'A cache miss on a public asset': 'Caching does not explain the security weakness in the payment endpoint.',
      'A compiler optimizing the signature': 'Compiler behavior does not make a captured valid request safe from reuse.',
    },
  },
  {
    id: 'distributed-queue-ordering',
    title: 'Queue Fork',
    difficulty: 'Advanced',
    xp: 660,
    category: 'Distributed Systems',
    concept: 'Message Delivery',
    prompt: 'Two workers process account events from a queue. An update can arrive twice and out of order. Which design best protects the final account balance?',
    choices: ['Make the handler idempotent and apply version checks', 'Trust network order and ignore duplicates', 'Retry forever until the queue is empty', 'Use a larger message payload'],
    correctAnswer: 'Make the handler idempotent and apply version checks',
    explanation: 'Idempotency makes duplicates harmless, while version checks reject stale events when delivery order is not guaranteed.',
    choiceExplanations: {
      'Make the handler idempotent and apply version checks': 'Correct. The consumer acknowledges the realities of at-least-once, unordered delivery.',
      'Trust network order and ignore duplicates': 'Distributed networks do not guarantee either property.',
      'Retry forever until the queue is empty': 'Retries do not repair stale ordering and can block progress.',
      'Use a larger message payload': 'More data does not establish ordering or duplicate safety.',
    },
  },
  {
    id: 'ml-drift-signal',
    ...
    concept: 'Production ML',
    prompt: 'A fraud model keeps its offline accuracy, but approval rates drop after a new product launch. Which investigation should come first?',
    choices: ['Compare production feature distributions with training data', 'Increase model depth immediately', 'Delete the newest training examples', 'Judge quality only from offline accuracy'],
    correctAnswer: 'Compare production feature distributions with training data',
    explanation: 'A product change can shift real-world inputs without changing the old benchmark. Feature and label drift checks reveal whether production no longer resembles training.',
    choiceExplanations: {
      'Compare production feature distributions with training data': 'Correct. Monitoring the input population tests the most likely production change first.',
      'Increase model depth immediately': 'A larger model can amplify the issue and skips diagnosis.',
      'Delete the newest training examples': 'Removing data without evidence can make the model less representative.',
      'Judge quality only from offline accuracy': 'Offline accuracy may remain stable while the live population changes.',
    },
  },
  {
    id: 'ai-agent-goal',
  ...
  {
    id: 'ds-structure-choice',
    title: 'Structure Choice',
    difficulty: 'Intermediate',
    xp: 360,
    category: 'Data Structures',
    concept: 'Choosing Structures',
    prompt: 'A service needs fast lookup by user ID, while preserving insertion order is not important. Which structure is the best default?',
    choices: ['Hash map', 'Stack', 'Queue', 'Binary tree with no ordering guarantee'],
    correctAnswer: 'Hash map',
    explanation: 'A hash map is designed for average constant-time lookup by key when ordering is not required.',
    choiceExplanations: {
      'Hash map': 'Correct. Keys map directly to values and provide fast average lookup.',
      'Stack': 'A stack is optimized for last-in, first-out access, not arbitrary key lookup.',
      'Queue': 'A queue is optimized for first-in, first-out processing.',
      'Binary tree with no ordering guarantee': 'Without an ordering or balancing property, lookup can degrade to a linear scan.',
    },
  },
  {
    ...
    category: 'Data Structures',
    concept: 'Trees',
    prompt: 'You need to process every node in a tree and do not know its maximum depth. Which traversal approach avoids risking call-stack overflow?',
    choices: ['An iterative traversal with an explicit stack', 'Always use recursive depth-first search', 'Inspect only the root node', 'Sort the node values first'],
    correctAnswer: 'An iterative traversal with an explicit stack',
    explanation: 'An explicit stack moves traversal state from the call stack into managed data, making deep or adversarial trees safer to process.',
    choiceExplanations: {
      'An iterative traversal with an explicit stack': 'Correct. It preserves depth-first behavior while controlling memory explicitly.',
      'Always use recursive depth-first search': 'Deep trees can exceed the language runtime call-stack limit.',
      'Inspect only the root node': 'The root does not represent the complete tree.',
      'Sort the node values first': 'Sorting does not traverse or discover the tree structure.',
    },
  },
  {
    id: 'ds-graph-bfs',
    ...
    category: 'Data Structures',
    concept: 'Graphs',
    prompt: 'In an unweighted social graph, you need the fewest connection hops between two people. Which algorithm is the natural first choice?',
    choices: ['Breadth-first search', 'Depth-first search only', 'Insertion sort', 'Binary search on names'],
    correctAnswer: 'Breadth-first search',
    explanation: 'Breadth-first search explores the graph level by level, so the first time it reaches a node it has found the fewest-edge path from the start.',
    choiceExplanations: {
